...
if (Sys.Date() == floor_date(Sys.Date(), 'week') + 1) {
  actual <- paste('epidemiologia/datos/dengue/DENGUE2_', format(floor_date(Sys.Date(), 'week') + 1, '%d%m%y'), '.txt', sep = '')
  anterior <- paste('epidemiologia/datos/dengue/DENGUE2_', format(floor_date(Sys.Date(), 'week') - 6, '%d%m%y'), '.txt', sep = '')
} else {
  anterior <- paste('epidemiologia/datos/dengue/DENGUE2_', format(floor_date(Sys.Date(), 'week') + 1, '%d%m%y'), '.txt', sep = '')
  actual <- paste('epidemiologia/datos/dengue/DENGUE2_', format(Sys.Date(), '%d%m%y'), '.txt', sep = '')
}
...
dengue_actual <- read_delim(actual, delim = '|', locale = locale(encoding = 'latin1'), comment = '', quote = '') %>% mutate(CERTIFICADO_DEFUNCION = as.numeric(CERTIFICADO_DEFUNCION), IDE_TEL = as.numeric(IDE_TEL), IDE_CP = as.numeric(IDE_CP)) %>% 
  bind_rows(
    read_delim('epidemiologia/datos/dengue/DENGUE2_20240225.txt', delim = '|', locale = locale(encoding = 'latin1'), comment = '', quote = '') %>% mutate(IDE_TEL = as.numeric(IDE_TEL), TELFONO_TRABAJO = as.numeric(TELFONO_TRABAJO))
    ) %>% 
  ...
  mutate(semana = paste(year(floor_date(FEC_INI_SIGNOS_SINT, "week") + 3), "-", str_pad(floor_date(FEC_INI_SIGNOS_SINT, "week"), pad = "0", width = 2, side = "left"), sep = ""),
...
nacional <- read_csv('epidemiologia/00_Pob_Mitad_1950_2070.csv', locale = locale(encoding = 'UTF-8'), comment = '', quote = '') %>% 
...
plot_ly(bol, x = ~ESTADO) %>%
  ...
  layout(title = '', legend = list(x = 1.1, y = 0.9, orientation = "v"),
         yaxis2 = list(title = "Incidencia por 100 000 habs", side = "right", showgrid = F, ticks="outside",
                       zerolinecolor = '#000000', overlaying = "y", range = c(0, 1.2 * max(nacional$IA, na.rm = T))),
...
poblaciones <- read_csv('epidemiologia/base_municipios_sonora.csv') %>% 
    group_by(MUN) %>%
  summarise(poblacion = sum(POB, na.rm = T)) %>%
  mutate(MUN = str_to_upper(MUN, locale = 'en'))
...
```{r, echo=FALSE, message=F, warning=F}
son <- st_read(dsn = "epidemiologia/muni_2018gw/muni_2018gw.shp", quiet = TRUE) %>%
...
dengue_ <- rbind(dengue_, completo)
