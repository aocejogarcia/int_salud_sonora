...
if (Sys.Date() == floor_date(Sys.Date(), 'week') + 1) {
  actual <- paste('data/DENGUE2_', format(floor_date(Sys.Date(), 'week') + 1, '%d%m%y'), '.txt', sep = '')
  anterior <- paste('data/DENGUE2_', format(floor_date(Sys.Date(), 'week') - 6, '%d%m%y'), '.txt', sep = '')
} else {
  anterior <- paste('data/DENGUE2_', format(floor_date(Sys.Date(), 'week') + 1, '%d%m%y'), '.txt', sep = '')
  actual <- paste('data/DENGUE2_', format(Sys.Date(), '%d%m%y'), '.txt', sep = '')
}
...
dengue_actual <- read_delim(actual, delim = '|', locale = locale(encoding = 'latin1'), comment = '', quote = '') %>% mutate(CERTIFICADO_DEFUNCION = as.numeric(CERTIFICADO_DEFUNCION), IDE_TEL = as.numeric(IDE_TEL), IDE_CP = as.numeric(IDE_CP)) %>% 
  bind_rows(
    read_delim('data/DENGUE2_20240225.txt', delim = '|', locale = locale(encoding = 'latin1'), comment = '', quote = '') %>% mutate(IDE_TEL = as.numeric(IDE_TEL), TELFONO_TRABAJO = as.numeric(TELFONO_TRABAJO))
    ) %>% 
...
nacional <- read_csv('data/00_Pob_Mitad_1950_2070.csv', locale = locale(encoding = 'UTF-8'), comment = '', quote = '') %>% 
...
poblaciones <- read_csv('data/base_municipios_sonora.csv') %>% 
    group_by(MUN) %>%
...
```{r, echo=FALSE, message=F, warning=F}
son <- st_read(dsn = "../covid/muni_2018gw.shp", quiet = TRUE) %>%
